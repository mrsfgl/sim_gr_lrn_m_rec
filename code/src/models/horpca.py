...
def horpca(Y, psi=1, beta=None, alpha=None, max_iter=100, err_tol=10**-5,
    ...
    sizes = Y.shape
    n = len(sizes)
    L, S, X, Lambda, psi, beta, alpha = init(Y, psi, beta, alpha)
    ...
    val, term = compute_obj(Y, L, X, S, Lambda, beta, alpha, nuc_norm)
    while iter != max_iter and err > err_tol:
        # X Update
        X, nuc_norm = soft_hosvd(L, Lambda[1], psi, 1/alpha[1])
        val, term = compute_obj(Y,L,X,S,Lambda,beta,alpha,nuc_norm)
        ...
        (Lambda,
         dual_err,
         change_lambda
         ) = update_Lambda(Y, Lambda, n, L, S, X, t_norm)
...
def compute_obj(Y, L, X, S, Lambda, beta, alpha, nuc_norm):
    ...
    for i in range(n):
        term[1] += nuc_norm[i]
        term[3] += alpha[1]/2*norm(L-X[i]-Lambda[1][i])**2
...
def init(Y, psi, beta, alpha):
    ...
    X = [np.zeros(sizes) for _ in range(n)]
    L = np.zeros(sizes)
    S = np.zeros(sizes)
    Lambda = [np.zeros(sizes), [np.zeros(sizes) for _ in range(n)]]
    return L, S, X, Lambda, psi, beta, alpha


def update_L(Y, S, X, Lambda, alpha):
    '''Updates variable L.'''
    sizes = Y.shape
    n = len(sizes)
    L = np.zeros(sizes)
    temp1 = alpha[0]*(Y-S-Lambda[0])
    temp2 = alpha[1]*sum(X[i] + Lambda[1][i] for i in range(n))
    L[~Y.mask] = (temp1[~Y.mask] + temp2[~Y.mask])/(alpha[0]+n*alpha[1])
    L[Y.mask] = temp2[Y.mask]/(n*alpha[1])
    return L


def update_Lambda(Y, Lambda, n, L, S, X, t_norm):
    lambda_update = L+S-Y
